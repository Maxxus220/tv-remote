...
bool button_timer_isr_handler(gptimer_handle_t timer, const gptimer_alarm_event_data_t *edata, void* user_ctx)
{
    return true;
}
...
extern "C"
{
    void app_main(void)
    {
        ...
        gpio_config_t button_conf {
            .pin_bit_mask = 1ULL << kButtonGPIONum,
        ...
        {
            struct ir_sensor_event event = {};
            if (xQueueReceive(s_ir_sensor_queue, &event, 10 / portTICK_PERIOD_MS) == pdTRUE)
            {
                if (event.val)
                {
                    if (message.full())
                    {
                        uint16_t val = convert_to_uint(gsl::span<bool, 15>(message.data(), message.size()));
                        printf("Message: %x\n", val);
                        message.clear();
                    }
